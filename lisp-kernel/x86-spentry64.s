...
__(tra(local_label(_threw_one_value)))
	...
	__(jmp _SPunbind_to)
        ...
        __(movq catch_frame.ffn(%temp0),%fn)
...
0:	__(movq -node_size(%imm0),%ra0)
	...
	__(cmpq %imm0,%rsp)
	__(jne 0b)
	__(lea -node_size(%rbp,%imm1),%rsp)
	__(movq node_size(%rbp),%ra0)
        __(movq -node_size(%rbp),%fn)
	__(movq (%rbp),%rbp)
        __(pushq %ra0)
	__(do_funcall())
...
_spentry(call_closure)
        new_local_labels()
        /* We need to preserve temp0 = next-method-context, */
        /* this code uses temp1, temp2 = ra0 which we use too */
        /* so temp3 is the next free register. */
        __(movq %nfn,%temp3)
        __(subq $fulltag_function-fulltag_misc,%temp3)
        __(vector_length(%temp3,%imm0))
...
local_label(insert_loop):               
        __(movq misc_data_offset(%temp3,%imm1),%arg_z)
        __(addq $node_size,%imm1)
...
local_label(vpush_remaining):  
        __(push misc_data_offset(%temp3,%imm1))
        __(addq $node_size,%imm1)
...
local_label(set_arg_x): 
        ...
        __(movq misc_data_offset(%temp3,%temp1),%arg_x)
        __(addl $fixnumone,%nargs)
        __(subq $fixnumone,%imm0)
...
local_label(set_arg_y): 
        __(subq $node_size,%temp1)
        __(movq misc_data_offset(%temp3,%temp1),%arg_y)
        __(addl $fixnumone,%nargs)
        __(subq $fixnum_one,%imm0)
...
local_label(set_arg_z): 
        __(subq $node_size,%temp1)
        __(movq misc_data_offset(%temp3,%temp1),%arg_z)
        __(addl $fixnumone,%nargs)
        __(subq $fixnum_one,%imm0)
        __(jne local_label(set_arg_y))
local_label(go):        
        __(push %ra0)
        __(movq misc_data_offset+node_size(%temp3),%nfn)
        __(jmp *function.entrypoint(%nfn))
local_label(regs_only):
        ...
        __(movq misc_data_offset-node_size(%temp3,%temp1),%arg_z)
        __(je local_label(rgo))
        __(cmpw $2*node_size,%imm0_w)
        __(movq misc_data_offset-(node_size*2)(%temp3,%temp1),%arg_y)
        __(je local_label(rgo))
        __(movq misc_data_offset-(node_size*3)(%temp3,%temp1),%arg_x)
local_label(rgo):
...
local_label(some_args):         
        ...
        __(cmpw $node_size,%imm0_w)
        __(movq misc_data_offset-node_size(%temp3,%temp1),%arg_y)
        __(je local_label(rgo))
        __(movq misc_data_offset-(node_size*2)(%temp3,%temp1),%arg_x)
        __(jmp local_label(rgo))
local_label(rtwo):     
        __(movq misc_data_offset-node_size(%temp3,%temp1),%arg_x)
        __(jmp local_label(rgo))
